...
with open('paramgrid_mutational_effects.txt', 'rb') as f:
...
for qd in qd_range:
    #print(nval)
    if 1-qd > 1e-6:
        sim = np.sum(simulation_precise_differentb(init,b1*0.99,b2,a,a1,q,qd,m,n,tim,ttr)[:,1])*1e-2
    elif 1-qd == 1e-6:
        sim = np.sum(simulation_precise_differentb(init,b1,b2,a,a1,q,qd,m,n,tim,ttr)[:,1])*1e-2
    all_results.append(sim)
    
output = '/home/labs/pilpel/gabril/COVID_merck/review/mutational_effects/'
