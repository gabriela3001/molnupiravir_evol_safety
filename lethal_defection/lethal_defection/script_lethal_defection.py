...
def merck_euler(population,dt,b,q,m,n,a,beta):
    ...
    dy[0] = y1 + dt*(b*(q**(m+n))-a-beta*y3)*y1
    dy[1] = y2 + dt*((b*(q**m)*(1-q**n))*y1 + y2*(b*(q**m)-a) - beta*y2*y3)
    dy[2] = dt*((y1+y2)*(b*(1-q**m)))
...
for d in along_d:
    ...
    for qd in along_qd:
        ...
        for ttr in along_ttr:
            control = simulation(init_pop,b,a,ap,q,q,m,n,d,tim,ttr)
            with_treatment = simulation(init_pop,b,a,ap,q,qd,m,n,d,tim,ttr)
            Y_control = np.sum(control[:,1])*1e-2
            Y_treatment = np.sum(with_treatment[:,1])*1e-2
            all_results.append({'m':m, 'n':n, 'd': d, 'ap':ap, 'qd':qd, 'ttr':ttr, 'Y_control':Y_control, 'Y_treatment':Y_treatment})
            
        print(time.time() - start_time)
        
output = '/home/labs/pilpel/gabril/COVID_merck/plos_revision/lethal_defection/'
